...
func TestChallenge(t *testing.T) {
	...
	nodeA.UnilateralExit(ledgerChannel, types.Challenge, ss, payments.Voucher{})
...
func TestCheckpoint(t *testing.T) {
	...
	nodeA.UnilateralExit(ledgerChannel, types.Challenge, oldState, payments.Voucher{})
	time.Sleep(2 * time.Second)
	nodeB.UnilateralExit(ledgerChannel, types.Checkpoint, newState, payments.Voucher{})
	time.Sleep(4 * time.Second)

	balanceNodeA, _ = infra.anvilChain.GetAccountBalance(testCase.Participants[0].Address())
	balanceNodeB, _ = infra.anvilChain.GetAccountBalance(testCase.Participants[1].Address())
	t.Log("Balance of Alice", balanceNodeA, "\nBalance of Bob", balanceNodeB)
	nodeB.UnilateralExit(ledgerChannel, types.Challenge, newState, payments.Voucher{})
...
func TestVirtualChannelWithVoucher(t *testing.T) {
	tc := TestCase{
	...
	// Create go-nitro nodes
	nodeA, _, _, _, _ := setupIntegrationNode(tc, tc.Participants[0], infra, []string{}, dataFolder)
	...
	fmt.Println(">>>>>LEDGER CHANNEL", ledgerChannel)
	...
	nodeBVoucher := <-nodeB.ReceivedVouchers()
	fmt.Println(">>>>>>RECIEVED VOUCHER", nodeBVoucher)
	...
	ss, err := nodeB.GetSupportedSignedState(virtualResponse.ChannelId)
	if err != nil {
		t.Fatal(err)
	}

	// v, _ := nodeB.GetVoucherIfAmountPresent(virtualResponse.ChannelId)
	// fmt.Printf(">>>>>VOUCHER>>> %+v", v)
	nodeB.UnilateralExit(ss.ChannelId(), types.Challenge, ss, nodeBVoucher)

	time.Sleep(5 * time.Second)

	ss, err = nodeB.GetSupportedSignedState(ledgerChannel)
	if err != nil {
		t.Fatal(err)
	}
	...
	nodeB.UnilateralExit(ss.ChannelId(), types.Challenge, ss, payments.Voucher{})
	...
	// 	t.Fatal(err)
	// }
	// nodeA.UnilateralExit(ss.ChannelId(), types.Challenge, ss)
	// time.Sleep(30 * time.Second)
...
func TestVirtualChannelWithoutVoucher(t *testing.T) {
	tc := TestCase{
		Description:       "Virtual channel test",
		Chain:             AnvilChainL1,
		MessageService:    TestMessageService,
	...
	ss, err := nodeA.GetSupportedSignedState(virtualResponse.ChannelId)
	if err != nil {
		t.Fatal(err)
	}
