...
func newL2ChainService(chain ethChain, startBlockNum uint64, bridge *Bridge.Bridge,
	...
	ecs := EthChainService{chain, nil, common.Address{}, caAddress, vpaAddress, txSigner, make(chan Event, 10), logger, ctx, cancelCtx, &sync.WaitGroup{}, tracker, nil, nil, make(chan error)}
