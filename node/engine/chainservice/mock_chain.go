...
import (
	...
	internalChain "github.com/statechannels/go-nitro/internal/chain"
...
func (mc *MockChain) SubmitTransaction(tx protocols.ChainTransaction) error {
	...
	case protocols.DepositTransaction:
		...
		for address := range tx.Deposit {
			event := NewDepositedEvent(tx.ChannelId(), mc.BlockNum, 0, address, h.Add(tx.Deposit)[address])
			eventsToBroadcast = append(eventsToBroadcast, event)
		}
	case protocols.WithdrawAllTransaction:
		for assetAddress := range h {
			event := NewAllocationUpdatedEvent(tx.ChannelId(), mc.BlockNum, 0, assetAddress, common.Big0)
			eventsToBroadcast = append(eventsToBroadcast, event)
		}
...
func NewAnvilChain() (*AnvilChain, AnvilChainOpts) {
	anvilCmd, _ := internalChain.StartAnvil()

	chain := AnvilChain{}
	chain.AnvilCmd = anvilCmd
	...
	naAddress, vpaAddress, caAddress, _ := internalChain.DeployContracts(context.Background(), chainUrl, chainAuthToken, chainPks[0])

	return &chain, AnvilChainOpts{
		ChainUrl:        chainUrl,
		ChainStartBlock: 0,
